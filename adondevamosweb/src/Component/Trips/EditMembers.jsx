...
function EditMembers() {
    ...
    const { saveMembers } = useTripDetailsApi();
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isMemberOrOwner()) {
            setMessageSnack("You are not authorized to edit this member list.");
            return;
        }
        ...
        try {
            if (
                ...
                await saveTripMembers();
                setMessageSnack("Members were saved successfully.");
            ...
            setMessageSnack(`Error updating members: ${error.message}`);
    ...
    const saveTripMembers = async () => {
        const lst = formTrip.members.map(member => ({
            "userid": member.user.id,
            "hide": member.hide || false
        }));
    ...
    const handleRemoveUser = (userId) => {
        const filteredList = formTrip.members.filter(item => item.user.id !== userId);
    ...
    if (errors.notMember) {
        return (
            <Box sx={{ p: 2 }}>
                ...
                >
                    You must be a member or owner of this trip to edit the member list.
                </Alert>
                <Button
    ...
    return (
        ...
        >
            ...
            >
                As a member or owner, you can add or remove members from this trip.
            </Alert>
            ...
            {errors.duplicateduser && (
                <Alert 
                    severity="warning"
                    onClose={() => setErrors(prev => ({ ...prev, duplicateduser: false }))}
                    sx={{
                        borderRadius: 0,
                        border: '2px solid #2C2C2C',
                        fontFamily: "'Press Start 2P', cursive",
